(() => {
  ...
  function renderSenders(){
    ...
    body.innerHTML=filtered.map(row=>{
      ...
      return `<tr>
        ...
        <td><div class="keyai-sender-assign ${assignAllowed?'':'readonly'}"><select data-keyai-sender-user="${esc(row.sender_id||'')}"${disabled}>${userOptions}</select><select data-keyai-sender-select="${esc(row.sender_id||'')}"${disabled}>${options}</select><select class="keyai-sender-mode" data-keyai-sender-mode="${esc(row.sender_id||'')}"${disabled}>${modeOptions}</select><button class="btn secondary" type="button" data-keyai-sender-save="${esc(row.sender_id||'')}"${disabled}>Save</button></div></td>
      </tr>`;
    }).join('');
  ...
  async function saveSenderAssignment(senderId,button){
    ...
    try{
      const result=await c.rpc('keysuite_v41300_assign_keybot_sender',{
