/* KeySuite V4.08.09 — role authority + fixed Brand / Series selection scope; Keylargo Baseplate remains a house product. */
(() => {
  'use strict';
  ...
  const HOUSE_PRODUCT_PAGES=new Set(['productBaseplate']);
  const OTHER_PRODUCT_PAGES=new Set(['productGws','productKeyplc','productManifold','productMotor','productCoupling']);
  ...
  const allowedFamilies=()=>{
    if(!brandSeriesLocked())return FAMILIES.slice();
    ...
    const api=window.KeySuiteV394410||window.KeySuiteV40001||window.KeySuiteV391||null,brands=api?.state?.brands||[],maps=api?.state?.mappings||[],wildBrands=new Set(wildcardKeys().map(k=>norm(k.split('|')[0]))),fromWild=[];
    wildBrands.forEach(id=>{
      const brand=brands.find(b=>norm(b?.id)===id);
      if(isMasterBrand(brand)){fromWild.push(...FAMILIES);return}
      maps.filter(m=>m&&m.active!==false&&norm(m.brand_id)===id).forEach(m=>{const f=upper(m.master_family);if(FAMILIES.includes(f))fromWild.push(f)});
    });
    return [...new Set([...explicit,...(fromWild.length?fromWild:FAMILIES)])];
  };
  const allowedBrandIds=()=>{
    if(!brandSeriesLocked())return [];
    return [...new Set(scopeKeys().map(k=>norm(k.split('|')[0])).filter(Boolean))];
  };
  function isBrandSeriesAllowed(brandId,family){
    if(!brandSeriesLocked())return true;
  ...
  function resolveBrandSeries(brandId='',family=''){
    ...
    if(!brandSeriesLocked())return {brandId:norm(brandId),family:fam};
  ...
  function filterBrandSeriesEntries(entries=[]){
    if(!brandSeriesLocked())return Array.isArray(entries)?entries:[];
  ...
  function canOpenPage(id){
    if(Object.prototype.hasOwnProperty.call(SELECTOR_PAGE_FAMILY,id)){
      ...
      return !brandSeriesLocked()||allowedFamilies().includes(family);
    }
    if(Object.prototype.hasOwnProperty.call(PRODUCT_PAGE_FAMILY,id)){
      if(!can('use_product'))return false;
      return !brandSeriesLocked()||allowedFamilies().includes(PRODUCT_PAGE_FAMILY[id]);
    }
    if(HOUSE_PRODUCT_PAGES.has(id))return can('use_product');
    if(OTHER_PRODUCT_PAGES.has(id))return can('use_product')&&!brandSeriesLocked();
  ...
  function applyVisibility(){
    try{
      ...
      const selectorVisible=can('use_selector')&&(!brandSeriesLocked()||allowedFamilies().length>0);
      ...
      Object.entries(SELECTOR_PAGE_FAMILY).forEach(([page,fam])=>setDisplay(`.nav-group[data-nav-group="selectorMenu"] button[data-page="${page}"]`,can('use_selector')&&(!brandSeriesLocked()||allowedFamilies().includes(fam)),'block'));
  ...
  async function loadScope(){
    ...
    try{
      const {data,error}=await db.rpc('keysuite_get_my_selection_scope_v40407');if(error)throw error;
      ...
      console.warn('[KeySuite V4.05.17] selection scope could not be loaded:',error);
      return state.scope;
    }
  }
  ...
  window.KeySuiteAuthority={version:VERSION,state,init,setAccess,reset,setScope,loadScope,snapshot,level,can,brandSeriesLocked,scopeKeys,allowedFamilies,allowedBrandIds,isBrandSeriesAllowed,resolveBrandSeries,filterBrandSeriesEntries,canOpenPage,applyVisibility,keyOf};
})();
