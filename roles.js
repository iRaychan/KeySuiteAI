(() => {
  ...
  function selectionEntries(){
    const api=multibrandApi(),brands=(api?.state?.brands||[]).filter(b=>b&&b.active!==false&&String(b.brand_key||'').toLowerCase()!=='keylargo'),maps=(api?.state?.mappings||[]).filter(m=>m&&m.active!==false),out=[];
    brands.forEach(brand=>{
      const master=String(brand.brand_type||'').toLowerCase()==='master'||String(brand.brand_key||'').toLowerCase()==='b.g.reich'||String(brand.brand_name||'').toLowerCase()==='b.g.reich';
      const fams=master?['CHC','ES']:[...new Set(maps.filter(m=>String(m.brand_id)===String(brand.id)).map(m=>String(m.master_family||'').toUpperCase()).filter(f=>['CHC','ES'].includes(f)))];
      fams.forEach(family=>out.push({brand,family,key:`${brand.id}|${family}`,series:String(api?.brandSeriesFor?.(brand,family)||family)}));
    });
    return out;
  }
  ...
  function updateSelectionScopeNote(){
    ...
    if(roleSelectionLocked(roleName)&&roleSelectionEnabled(roleName))note.innerHTML='<b>Assigned visibility:</b> only the Owner can assign which Brands / Series this user can see. The user may change settings only inside that assigned list.';
    else note.textContent=isOwner()?'Owner may pre-assign a Brand / Series visibility scope here. It becomes mandatory when the selected role has “Change Brand / Series = No”.':'Brand / Series visibility is Owner-assigned and cannot be changed here.';
  }
  function renderSelectionScopeEditor(user=editingUser){
    const grid=el('roleSelectionScopeGrid');if(!grid)return;const entries=selectionEntries(),selected=new Set(scopeKeys(user?.selection_scope)),ownerCanAssign=isOwner();
    if(selectionScopeError){grid.innerHTML=`<div class="role-note" style="color:#991b1b">Selection authority database is not ready: ${esc(selectionScopeError)}. Run V40407_SUPABASE_MIGRATION.sql.</div>`;updateSelectionScopeNote();return}
    if(!entries.length){grid.innerHTML='<div class="role-note">Brand data is still loading. Reopen this user after Brand data is available.</div>';updateSelectionScopeNote();return}
    const grouped=new Map();entries.forEach(entry=>{const id=String(entry.brand.id);if(!grouped.has(id))grouped.set(id,{brand:entry.brand,entries:[]});grouped.get(id).entries.push(entry)});
    grid.innerHTML=[...grouped.values()].map(group=>{
      const brandId=String(group.brand.id),allKey=`${brandId}|*`,allSelected=selected.has(allKey);
      return `<div class="role-selection-brand" data-selection-brand-group="${esc(brandId)}"><label class="role-selection-brand-all"><input type="checkbox" data-selection-brand-all="${esc(allKey)}" ${allSelected?'checked':''} ${ownerCanAssign?'':'disabled'}> <b>${esc(group.brand.brand_name||'Brand')}</b><span>All Series</span></label><div class="role-selection-series-list">${group.entries.map(entry=>`<label><input type="checkbox" data-selection-scope-key="${esc(entry.key)}" data-selection-brand-id="${esc(brandId)}" ${(allSelected||selected.has(entry.key))?'checked':''} ${(allSelected||!ownerCanAssign)?'disabled':''}> <span>${esc(entry.series||entry.family)}</span></label>`).join('')}</div></div>`;
    }).join('');
  ...
  async function load(){
    ...
    try{
      const [userResult,auditResult,prefixResult,scopeResult]=await Promise.all([db.rpc('keysuite_list_role_users'),db.rpc('keysuite_list_role_audit',{p_limit:30}),isOwner()?db.rpc('keysuite_list_quotation_prefixes_v225'):Promise.resolve({data:[],error:null}),db.rpc('keysuite_list_user_selection_scopes_v40407')]);
  ...
  function permissionCell(row,roleName){
    const fixed=roleName==='owner'&&['key_dashboard','keyai_access','keyai_openai_control','keyai_sender_assign','manage_roles','use_quick_selection','use_selector','use_product','choose_brand_series','own_profile'].includes(row.key);
  ...
  function collectPermissionMatrix(){
    ...
    matrix.owner={...(matrix.owner||{}),key_dashboard:'full',keyai_access:'full',keyai_openai_control:'full',keyai_sender_assign:'full',manage_roles:'full',use_quick_selection:'full',use_selector:'full',use_product:'full',choose_brand_series:'full',own_profile:'full'};
    return matrix;
  }
  async function savePermissions(){
    ...
    if(lockedRoles.length&&selectionScopeError){setMessage('rolePermissionsMessage',`Selection authority database is not ready: ${selectionScopeError}. Run V40407_SUPABASE_MIGRATION.sql before locking Brand / Series.`,'error');return}
  ...
  async function save(event){
    ...
    try{
      ...
      if(isOwner()){button.textContent='Saving Brand / Series visibility…';const scopeResult=await client().rpc('keysuite_set_user_selection_scope_v40407',{p_email:email,p_scope:selectionScope});if(scopeResult.error)throw new Error(`${scopeResult.error.message||scopeResult.error}. For V4.05.14 multi-brand / All Series permissions, run V40514_SUPABASE_MIGRATION.sql.`);}
