...
(()=>{
  ...
  window.KEYSUITE_VERSION='4.17.10';
  window.KEYSUITE_BUILD='V4.17.10';
  const VERSION='41710';
  ...
  function updateVisibleVersion(){try{document.title='KeySuite V4.17.10';document.querySelectorAll('.suite-version,.brand small,.auth-brand small').forEach(el=>{const t=String(el.textContent||'');if(/KeySuite|Full Suite|V\d/i.test(t))el.textContent=t.includes('Full Suite')?'Full Suite V4.17.10':'V4.17.10'});document.documentElement.setAttribute('data-keysuite-version','4.17.10')}catch(_){} }
  (async()=>{
    for(const f of FILES){try{await load(f)}catch(e){console.error('[KeySuite V4.17.10]',e)}}
    ...
    if(!window.KeySuiteV40001){
      for(const f of ['v40001-multibrand.js','v40001-selector-brand.js','v40001-customer-brand-settings.js','v40001-product-series-overhaul.js']){
        try{const s=document.createElement('script');await new Promise((resolve,reject)=>{s.src=`./${f}?v=${VERSION}&brand-retry=1`;s.async=false;s.onload=resolve;s.onerror=()=>reject(new Error(`Unable to recover ${f}`));(document.head||document.documentElement).appendChild(s)})}catch(e){console.error('[KeySuite V4.17.10]',e)}
      }
    }
    const repair=()=>{try{window.KeySuiteV40001?.repairBrandUi?.({reload:!!window.KEYSUITE_PROFILE?.company_id&&!window.KeySuiteV40001?.state?.coreReady})}catch(e){console.warn('[KeySuite V4.17.10]',e)}};
    repair();setTimeout(repair,500);setTimeout(repair,2000);
    updateVisibleVersion();
    try{window.dispatchEvent(new CustomEvent('KEYSUITE_V41300_RUNTIME_READY',{detail:{version:'4.17.10'}}));window.dispatchEvent(new CustomEvent('KEYSUITE_V41223_RUNTIME_READY',{detail:{version:'4.17.10'}}));window.dispatchEvent(new CustomEvent('KEYSUITE_V41223_BRAND_RECOVERY'))}catch(_){ }
  })();
})();
