(() => {
  ...
  function calculatePrice(priceBook,variant,cat=category(),family='CHC',options={}){
    ...
    const candidates=currencyCandidates(priceBook,options.rarityBook||{},variant,family).filter(row=>allowed.has(row.currency));if(!candidates.length)return null;
    ...
    const chosen=pool.reduce((best,row)=>!best||row.baseMyr>best.baseMyr?row:best,null),priceContext=context(options),company=companyFactors(pricingMode,cat,family,priceContext.customer),rarity=normalizeRarity(options.rarity||chosen.rarity),fixedPrice=rarity==='fixed';
    if(fixedPrice){
